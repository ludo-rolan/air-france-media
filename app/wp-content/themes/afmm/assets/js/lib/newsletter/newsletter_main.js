jQuery(document).ready(function ($) {
	...
	if ($("#signupForm_nl").length > 0) {
		...
		var validation_msgs = {
			...
			OPTIN_CHECKBOX: "Ce Champ est obligatoire",
		};
		...
		if (lang != 'undefined' && lang.indexOf("en") !== -1) {
			validation_msgs = {
				...
				OPTIN_CHECKBOX: "This field is required",
			};
		...
		$("#signupForm_nl").validate({
			...
			rules: {
				...
				SECTEUR: "required",
				PROFESSION: "required",
				OPTIN_CHECKBOX: "required",
			},
			errorPlacement: function (error, element) {
				switch (element.attr("name")) {
					case "OPTIN_CHECKBOX":
						error.appendTo(".choice-1-error");
						break;
					default:
	...
	$('#signupForm_nl').on('submit', function (e) {
		...
		if ($("#signupForm_nl").valid()) {
			...
			var langue =  $('#langue_nl, #email').val();
