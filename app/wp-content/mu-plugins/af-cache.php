<?php
//class pour mise en cache des script
...
//class pour mise en cache des styles
 ...
 */
	 ...
	 */
	public function get_data_from_cache( $key , $group , $duration, $callback, $use_key_prefix = true ){
		if( $use_key_prefix ){
			$key = get_cache_key($key);
		}else{
		...
		if ( !empty($cached_data) && !isset($_GET['disable_cache'])) {
			...
			$data = $callback();
			if(isset($_GET['debug_objet_cache'])){
				echo "not_from_cache  key = $key , goup $group , duration $duration  " ;
			}
...
function echo_from_cache( $key , $group , $duration, $callback){
	$af_cache_object = AF_Cache::get_instance();
	$af_cache_object->echo_from_cache( $key , $group , $duration, $callback);
} 

function get_data_from_cache( $key , $group , $duration, $callback, $use_key_prefix = true ){
	$af_cache_object = AF_Cache::get_instance();
	return $af_cache_object->get_data_from_cache( $key , $group , $duration, $callback, $use_key_prefix);
} 

function get_cache_key($key){
	$af_cache_object = AF_Cache::get_instance();
	return $af_cache_object->get_cache_key( $key );
}
...
function af_send_cache_headers($duration){
...
function af_setup_caching_sitemap($request){
	switch ( $request['feed'] ) {
	...
	af_send_cache_headers($cache);
}

add_action( 'sitemap_request'  , 'af_setup_caching_sitemap');
/**
 * delete_cache_object_data Pour supprimer le cache objet dont la clé est de type [id_blog]_[cle_cache] 
 * $key - la clé de cache
 * $group - le groupe de cache
...
function delete_cache_object_data( $key, $group){
	$af_cache_object = AF_Cache::get_instance();
	return $af_cache_object->delete_cache_object_data( $key, $group);
}
...
add_action("wp", function(){
	...
	if(is_single()){
		...
		if($time_diff >= 60*60*24*30*3 ){
			...
			af_send_cache_headers(60*60*24*30);
		}else{
			//4 heures de cache pour les articles de moins de 3 mois.
			af_send_cache_headers(60*60*4);
		}
	}
...
function af_cache_get($id, $group = 'default') {
...
function af_cache_set($id, $data, $group = 'default', $expire = 0) {
	
	return wp_cache_set($id, $data, $group, $expire) ;
}

function af_cache_delete($id, $group = 'default') {
